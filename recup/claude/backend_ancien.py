...
def find_matching_titles_in_csv(excel_titles, hierarchical_data):
    """Trouve les titres du CSV qui correspondent aux titres Excel sélectionnés - VERSION AMÉLIORÉE."""
    matching_titles = []
    
    print(f"Recherche de correspondances pour {len(excel_titles)} titres Excel")
    
    for excel_title in excel_titles:
        excel_title_upper = excel_title.upper()
        print(f"Traitement du titre Excel: '{excel_title_upper}'")
        ...
        if excel_title_upper in TITLE_SUBTITLE_DICTIONARY:
            ...
            print(f"Patterns trouvés pour '{excel_title_upper}': {patterns}")
            
            # Cherche dans les titres du CSV
            for title_key, title_data in hierarchical_data.items():
                csv_title = title_data.get("title", "").upper()
                print(f"Comparaison avec CSV titre: '{csv_title}'")
                
                # Correspondance par patterns - AMÉLIORATION : recherche de correspondance partielle
                for pattern in patterns:
                    pattern_upper = pattern.upper()
                    
                    # Correspondance exacte
                    if pattern_upper in csv_title:
                        ...
                        print(f"✓ CORRESPONDANCE EXACTE trouvée: pattern '{pattern}' dans '{csv_title}'")
                        break
                    
                    # Correspondance par mots-clés multiples
                    pattern_words = pattern_upper.split()
                    csv_words = csv_title.split()
                    
                    matches = 0
                    for pattern_word in pattern_words:
                        if len(pattern_word) > 2:  # Ignore les mots très courts
                            for csv_word in csv_words:
                                if pattern_word in csv_word or csv_word in pattern_word:
                                    matches += 1
                                    break
                    
                    # Si au moins 60% des mots correspondent
                    if matches >= len(pattern_words) * 0.6 and matches >= 1:
                        matching_titles.append({
                            "title_key": title_key,
                            "title_data": title_data,
                            "excel_title": excel_title,
                            "match_type": "pattern_partial",
                            "pattern": pattern,
                            "match_score": matches / len(pattern_words)
                        })
                        print(f"✓ CORRESPONDANCE PARTIELLE trouvée: {matches}/{len(pattern_words)} mots correspondent")
                        break
        
        # Si aucune correspondance via le dictionnaire, recherche directe
        if not any(m["excel_title"] == excel_title for m in matching_titles):
            print(f"Aucune correspondance dictionnaire pour '{excel_title_upper}', recherche directe...")
            
            for title_key, title_data in hierarchical_data.items():
                csv_title = title_data.get("title", "").upper()
                
                # Correspondance par mots-clés directs
                excel_words = [word for word in excel_title_upper.split() if len(word) > 3]
                
                for word in excel_words:
                    if word in csv_title:
                        matching_titles.append({
                            "title_key": title_key,
                            "title_data": title_data,
                            "excel_title": excel_title,
                        ...
                        print(f"✓ CORRESPONDANCE DIRECTE trouvée: mot-clé '{word}' dans '{csv_title}'")
                        break
    
    print(f"Total correspondances trouvées: {len(matching_titles)}")
    for match in matching_titles:
        print(f"  - {match['excel_title']} → {match['title_data']['title']} ({match['match_type']})")
    
    return matching_titles

def find_matching_subtitles_in_csv(excel_subtitle, excel_titles, hierarchical_data):
    """Trouve les sous-titres du CSV qui correspondent au sous-titre Excel - VERSION AMÉLIORÉE."""
    matching_subtitles = []
    ...
    print(f"Recherche de sous-titres pour: '{excel_subtitle_upper}'")
    ...
    for excel_title in excel_titles:
        ...
        if excel_title_upper in TITLE_SUBTITLE_DICTIONARY:
            ...
            print(f"Mapping sous-titres pour '{excel_title_upper}': {list(subtitle_mapping.keys())}")
            
            # Trouve les sous-titres correspondants
            for subtitle_key, csv_subtitles in subtitle_mapping.items():
                subtitle_key_upper = subtitle_key.upper()
                
                # Correspondance par clé de sous-titre
                if subtitle_key_upper in excel_subtitle_upper or excel_subtitle_upper in subtitle_key_upper:
                    print(f"Correspondance par clé: '{subtitle_key_upper}' ↔ '{excel_subtitle_upper}'")
                    ...
                    for title_key, title_data in hierarchical_data.items():
                        for subtitle_full_key, subtitle_data in title_data["subtitles"].items():
                            ...
                            for target_subtitle in csv_subtitles:
                                target_upper = target_subtitle.upper()
                                
                                # Correspondance exacte
                                if target_upper in csv_subtitle:
                                    ...
                                    print(f"✓ Sous-titre EXACT trouvé: '{target_subtitle}' dans '{csv_subtitle}'")
                                    break
                                
                                # Correspondance partielle par mots-clés
                                target_words = [w for w in target_upper.split() if len(w) > 2]
                                csv_words = [w for w in csv_subtitle.split() if len(w) > 2]
                                
                                matches = 0
                                for target_word in target_words:
                                    for csv_word in csv_words:
                                        if target_word in csv_word or csv_word in target_word:
                                            matches += 1
                                            break
                                
                                if matches >= len(target_words) * 0.5 and matches >= 1:
                                    matching_subtitles.append({
                                        "title_key": title_key,
                                        "subtitle_key": subtitle_full_key,
                                        "subtitle_data": subtitle_data,
                                        "match_type": "partial",
                                        "match_score": matches / len(target_words)
                                    })
                                    print(f"✓ Sous-titre PARTIEL trouvé: {matches}/{len(target_words)} mots correspondent")
                                    break
    
    # Recherche directe par correspondance partielle si aucun résultat
    if not matching_subtitles:
        print("Aucune correspondance dictionnaire, recherche directe...")
        ...
        for title_key, title_data in hierarchical_data.items():
            for subtitle_key, subtitle_data in title_data["subtitles"].items():
                ...
                excel_words = [w for w in excel_subtitle_upper.split() if len(w) > 3]
                
                for word in excel_words:
                    if word in csv_subtitle:
                        matching_subtitles.append({
                            "title_key": title_key,
                        ...
                        print(f"✓ Sous-titre DIRECT trouvé: mot-clé '{word}' dans '{csv_subtitle}'")
                        break
    
    print(f"Total sous-titres trouvés: {len(matching_subtitles)}")
    return matching_subtitles
...
def debug_title_matching(excel_titles, hierarchical_data):
    """Fonction de debug pour analyser les correspondances."""
    print("=== DEBUG CORRESPONDANCE TITRES ===")
    
    print(f"Titres Excel reçus: {excel_titles}")
    print(f"Titres disponibles dans le dictionnaire: {list(TITLE_SUBTITLE_DICTIONARY.keys())}")
    
    print("\nTitres dans le CSV hiérarchique:")
    for title_key, title_data in hierarchical_data.items():
        csv_title = title_data.get("title", "")
        print(f"  - {title_key}: '{csv_title}'")
        
        # Affiche aussi les sous-titres
        for subtitle_key, subtitle_data in title_data["subtitles"].items():
            csv_subtitle = subtitle_data.get("subtitle", "")
            print(f"    → {subtitle_key}: '{csv_subtitle}'")
...
def search():
    ...
    debug_title_matching(selected_titles, hierarchical_data)  # CORRIGÉ: passer les bons paramètres
